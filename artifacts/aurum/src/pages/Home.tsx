...
export default function Home() {
  ...
  return (
    <div className="w-full bg-white text-black min-h-[100dvh] pt-[64px]">
      ...
      {/* 2. HERO */}
      <section className="h-[calc(100vh-64px)] min-h-[600px] grid grid-cols-1 lg:grid-cols-2 relative overflow-hidden" data-testid="hero-section">
        <div className="flex flex-col justify-center px-8 lg:pl-20 lg:pr-16 py-20 z-10 bg-white lg:bg-transparent">
          <motion.div initial={{ opacity: 0, y: 32 }} animate={{ opacity: 1, y: 0 }} transition={{ ease: [0.4, 0, 0.2, 1], duration: 0.8, delay: 0.2 }}>
            <p className="text-[11px] text-[#999999] tracking-[0.25em] uppercase mb-6">SS26 Collection</p>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 32 }} animate={{ opacity: 1, y: 0 }} transition={{ ease: [0.4, 0, 0.2, 1], duration: 0.8, delay: 0.4 }}>
            <h1 className="font-serif text-[clamp(52px,5.5vw,84px)] leading-[1.05] tracking-tight mb-8">
              <span className="italic font-light">Discover your</span><br/>
              <span className="font-medium">Signature Style</span>
            </h1>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 32 }} animate={{ opacity: 1, y: 0 }} transition={{ ease: [0.4, 0, 0.2, 1], duration: 0.8, delay: 0.6 }}>
            <p className="text-[14px] font-light leading-[1.8] text-[#333333] max-w-[380px] mb-12">
              Meticulously crafted for those who understand that true luxury lies in the details. Timeless pieces that transcend seasons.
            </p>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 32 }} animate={{ opacity: 1, y: 0 }} transition={{ ease: [0.4, 0, 0.2, 1], duration: 0.8, delay: 0.8 }} className="flex flex-row gap-4">
            <Link href="/products">
              <button className="bg-black text-white hover:bg-white hover:text-black hover:border-black border border-transparent px-8 py-3.5 text-[11px] uppercase tracking-[0.15em] transition-colors duration-300 w-full sm:w-auto">
                Shop Collection
              </button>
            </Link>
            <Link href="/products?category=new">
              <button className="border border-black text-black hover:bg-black hover:text-white px-8 py-3.5 text-[11px] uppercase tracking-[0.15em] transition-colors duration-300 w-full sm:w-auto">
                Explore Looks
              </button>
            </Link>
          </motion.div>
        </div>
        <div className="hidden lg:block relative" style={{ background: 'linear-gradient(160deg, #f0ede8 0%, #e8e2d9 40%, #ddd6c8 100%)' }}>
          <div className="absolute top-10 left-10 bg-white/80 backdrop-blur border border-[#EAEAEA] px-4 py-2 flex flex-col z-20">
            <span className="text-[10px] tracking-[0.2em] uppercase">New Collection · 2026</span>
          </div>
          <p className="absolute top-10 right-10 text-[11px] tracking-[0.2em] uppercase text-[#999999]">SS 2026</p>
          <p className="absolute bottom-10 left-10 text-[11px] tracking-[0.2em] uppercase text-[#999999] origin-bottom-left -rotate-90 whitespace-nowrap">Editorial Collection</p>
          <FigureSVG figType="coat" ca="#c9bfb0" cb="#1e1e1e" className="absolute bottom-0 right-[8%] w-[55%] h-[90%] opacity-85" />
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-[2px] bg-white pt-[2px]">
        {/* Card 1 */}
        <div onClick={() => setLocation('/products?category=clothing')} className="group cursor-pointer relative md:col-span-2 min-h-[400px] md:min-h-[500px] overflow-hidden flex flex-col justify-end p-10" style={{ background: 'linear-gradient(135deg, #e8e0d5, #d5c9b8, #c2b49e)' }}>
        ...
        {/* Card 3 */}
        <div onClick={() => setLocation('/products?category=accessories')} className="group cursor-pointer relative min-h-[400px] md:min-h-[500px] overflow-hidden flex flex-col justify-end p-10" style={{ background: 'linear-gradient(135deg, #f0ebe3, #e0d8cc)' }}>
          <motion.div className="absolute inset-0 z-0 origin-center" whileHover={{ scale: 1.05 }} transition={{ duration: 0.7, ease: [0.4, 0, 0.2, 1] }}>
      ...
      <section className="my-20 mx-8 grid grid-cols-1 lg:grid-cols-2 gap-[2px] min-h-[600px] bg-white">
        <div className="relative p-12 lg:p-20 flex flex-col justify-end group cursor-pointer overflow-hidden min-h-[400px]" style={{ background: 'linear-gradient(135deg, #f0ebe3, #e0d5c8)' }}>
        ...
        <div className="grid grid-rows-2 gap-[2px]">
          ...
          <div className="bg-[#EAEAEA] p-12 flex flex-col justify-center relative overflow-hidden group cursor-pointer">
            <div className="relative z-20">
              ...
              <div className="flex items-center gap-4 text-[11px] uppercase tracking-[0.2em]">
                <span className="border-b border-black pb-1">Join Aurum</span>
                <span className="group-hover:translate-x-2 transition-transform duration-300">→</span>
              </div>
            </div>
          </div>
