...
export default function Products() {
  ...
  // Build API params — category/badge logic
  ...
  const { data: apiProducts, isLoading } = useListProducts({
    ...
    sortBy: sortBy !== 'featured' ? (sortBy as 'newest' | 'price-asc' | 'price-desc' | 'name-asc') : undefined,
  });

  // Fallback to hardcoded data when API is unavailable
  const useApi = apiProducts !== undefined;
  let products = useApi ? apiProducts : hardcodedProducts;

  // Client-side filtering for fallback data (when API unavailable)
  if (!useApi) {
      ...
      <div className="max-w-[1600px] mx-auto px-8">
        ...
        <div className="mb-12 flex items-end justify-between">
          ...
          <div className="flex items-center gap-2">
                ...
                >
                  <div className="relative">
                    <input
                      ...
                      className="w-full border border-black bg-white px-4 py-2.5 pr-8 text-[12px] placeholder:text-[#999] outline-none"
                    />
                    {searchQuery && (
                      <button
                        onClick={() => setSearchQuery('')}
                        className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[#999] hover:text-black"
                      >
                        <X size={12} />
                      </button>
                    )}
            ...
            <button
              ...
              className={`flex items-center justify-center w-10 h-10 border transition-colors ${searchOpen ? 'bg-black text-white border-black' : 'border-[#EAEAEA] hover:border-black'}`}
            >
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-10">
          <div className="flex gap-6 border-b border-[#EAEAEA] pb-0 overflow-x-auto no-scrollbar w-full md:w-auto">
            {categories.map(cat => (
              <button
                ...
                } ${cat.id === 'sale' ? (activeCategory !== cat.id ? 'text-black' : '') : ''}`}
              >
                {cat.label}
          ...
          <div className="flex items-center gap-4 shrink-0">
            ...
            <button
              ...
              className={`flex items-center gap-2 border px-4 py-2.5 text-[11px] uppercase tracking-[0.1em] transition-colors ${filtersOpen || hasActiveFilters ? 'bg-black text-white border-black' : 'border-[#EAEAEA] hover:border-black'}`}
            >
        ...
        {isLoading ? (
          ...
          <div className="py-32 text-center">
            ...
            {searchQuery && (
              <p className="text-[13px] text-[#999] mt-2">No results for "{searchQuery}"</p>
            )}
            <button
              onClick={() => { setActiveCategory('all'); setPriceRange([0, 100000]); setSearchQuery(''); setSortBy('featured'); }}
              className="mt-6 border border-black px-8 py-3 text-[11px] uppercase tracking-[0.15em] hover:bg-black hover:text-white transition-colors"
            >
              Clear Filters
            </button>
          </div>
